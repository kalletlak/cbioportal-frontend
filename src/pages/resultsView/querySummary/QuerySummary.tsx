...
export default class QuerySummary extends React.Component<{ queryStore:QueryStore, store: ResultsViewPageStore }, {}> {
    ...
    private get singleStudyUI() {
        return <div>
            <h4><StudyLink study={this.props.store.userQueryStudies.result[0]}/></h4>
            <span>
    ...
    private get multipleStudyUI() {
        return <div>
            <span>
                Querying {this.props.store.samples.result.length} samples in {this.props.store.userQueryStudies.result.length} studies
                 &nbsp;
                 <DefaultTooltip
                     placement='bottom'
    ...
    private get studyList(){
        ...
        return (<div className="cbioportal-frontend">
                ...
                {
                    this.props.store.userQueryStudies.result.map((study:CancerStudy)=>{
    ...
    render() {
        ...
        if (!this.props.store.totalAlterationStats.isError && !this.props.store.userQueryStudies.isError) {


            const loadingComplete = this.props.store.totalAlterationStats.isComplete && this.props.store.userQueryStudies.isComplete;
                ...
                <div>
                    <div className="query-summary">
                        <div className="query-summary__leftItems">
                            ...
                            {
                                (loadingComplete) && ((this.props.store.userQueryStudies.result.length === 1) ? this.singleStudyUI : this.multipleStudyUI)
                            }
                        </div>
                        {
